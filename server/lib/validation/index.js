...
class Validator {
  ...
  isValidEmail (email) {
    ...
    const errors = this._getErrorSignature();
    return new Promise((resolve, reject) => {
      const validEmailRegex = new RegExp(/^[\w-\.]+@([\w-]+\.)+[\w-]+$/);
  ...
  isTextareaNotEmpty (text) {
    ...
    const errors = this._getErrorSignature();
    return new Promise((resolve, reject) => {
      const notEmptyRegex = new RegExp(/[a-zA-Z]+/);
  ...
  isValidContactName (contactName) {
    ...
    const errors = this._getErrorSignature();
    return new Promise((resolve, reject) => {
      const validNameRegex = new RegExp(/^[AÀÁÂÃÄÅĀĂĄǺaàáâãäåāăąǻÆǼæǽBbCcçćĉċčDÞĎĐdþďđEÈÉÊËĒĔĖĘĚeèéêëēĕėęěFfGĜĞĠĢgĝğġģHĤĦhĥħIÌÍÎÏĨĪĬĮİiìíîïĩīĭįJĴjĵKĶkķLĹĻĽĿŁlĺļľŀłMmNÑŃŅŇŊnñńņňŋOÒÓÔÕÖØŌŎŐǾoòóôõöøōŏőǿŒœPpQqRŔŖŘrŕŗřSŚŜŞŠsśŝşšTŢŤŦtţťŧUÙÚÛÜŨŪŬŮŰŲuùúûüũūŭůűųVvWŴẀẂẄwŵẁẃẅXxYỲÝŶŸyỳýŷÿZŹŻŽzźżž&@£$€¥*=#%+‘ʼ'()\/\[\]{}<>!«»?“ˮ\"0123456789.,:;\–\-  \\r\\n]*$/);
  ...
  isValidDiscrepancyTypeSelection (discrepancies) {
    ...
    return new Promise((resolve, reject) => {
      try {
        if (typeof discrepancies.discrepancy === 'undefined' || discrepancies.discrepancy === null || discrepancies.discrepancy === '') {
          errors.stack.discrepancy = errorManifest.discrepancy.empty;
          reject(errors);
        } else {
          resolve(true);
        }
